...
int main(int argc, char **argv)
{
	...
	int obsnchan_num = 0;
	int fenchan_num = 0;
	int n_samps_per_pkt = 0; // Number of time samples per packet
	int piperblk_num = 0;
	int n_samp = 0;
	int n_sti = 0;
	...
	if (raw_file_flag == 1)
	{
		...
		{
			n_blocks = 63; // 125; // For 5.12 seconds of data // 153; // For 5 seconds worth of data
			n_nodes = 32;
			n_input = VLASS_N_INPUT;
			coarse_chan_bw = 1; // 1 MHz
			node_bw = 64;		// 64 MHz
			...
			else if (spec_flag == 1)
			{
				...
				n_ant_config = N_ANT / 2;
				n_chan = 1;
				nt = 5120000; // 5120000; // 5013504;
				n_win = 40;	  // 4;	  // 10000; // 2, 80, 10000
				n_time_int = 1;
		...
		tbin_num = (1 / coarse_chan_bw) * 1e-6;
		blksize = 2 * n_pol * n_sim_ant * (n_chan * n_subbands) * ((int)nt / n_blocks);
		obsnchan_num = n_sim_ant * (n_chan * n_subbands);
		fenchan_num = n_nodes * n_chan * n_subbands;
		n_samps_per_pkt = 1; // 256; // Number of time samples per packet
		piperblk_num = ((int)nt / n_blocks) / n_samps_per_pkt;
		n_samp = nt / n_win;
		...
		for (int f = 0; f < num_files; f++)
		{
			...
			input_file = fopen(sim_raw_filename, "ab");
	...
	char *obsid[1] = {OBSID_STR};
	// strcpy(obsid, OBSID_STR);
	...
	*/
	char *src_names_str[3] = {"JBLAH-BLAH.B00", "JBLAH-BLAH.B01", "JBLAH-BLAH.B02"};

	if (telescope_flag == 0)
	{
		nants_bfr5 = NANTS_MK;
		nbeams_bfr5 = NBEAMS_MK;
		n_times_bfr5 = NTIMES_BFR5;
		if (strcmp(mode_flag, "1k") == 0)
		{
	...
	{
		nants_bfr5 = 22; // NANTS_VLA;
		if (spec_flag == 0)
		{
			nbeams_bfr5 = 3; // NBEAMS_VLAR;
		}
		else if (spec_flag == 1)
		{
			nbeams_bfr5 = NBEAMS_VLAD;
		}
		nchan_bfr5 = 544; // NCHAN_VLA;
		n_times_bfr5 = NTIMES_VLA_BFR5;
	}
	int delays_elements = n_times_bfr5 * nbeams_bfr5 * nants_bfr5;
	int rates_elements = n_times_bfr5 * nbeams_bfr5 * nants_bfr5;

	// Allocate memory for array
	cal_all_data = malloc(nchan_bfr5 * NPOL_BFR5 * nants_bfr5 * sizeof(complex_t));
	delays_data = malloc(n_times_bfr5 * nbeams_bfr5 * nants_bfr5 * sizeof(complex_t));
	rates_data = malloc(n_times_bfr5 * nbeams_bfr5 * nants_bfr5 * sizeof(complex_t));
	time_array_data = malloc(n_times_bfr5 * sizeof(double));
	...
	for (int i = 0; i < nchan_bfr5; i++)
	{
		for (int j = 0; j < NPOL_BFR5; j++)
		{
			for (int k = 0; k < nants_bfr5; k++)
			{
				...
				cal_all_data[cal_all_idx(k, j, i, nants_bfr5, NPOL_BFR5)].im = 0;
	...
	if (sim_data_flag == 8)
	{
		if (raw_file_flag == 1)
		{
			printf("Sim data flag 8 computing time_array and delay arrays\n");
			int sim_delay_flag = 0;
			double synctime = 160328378; // Same synctime as in RAW file
			double pktidx = 0;
			for (int t = 0; t < n_times_bfr5; t++)
			{
